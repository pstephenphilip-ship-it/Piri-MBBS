...
(function (root) {
  ...
  SCENES.push({
    ...
    build: function (scene) {
      ...
      var pn = panelShell('My Notes · Acute Coronary Syndrome');
      pn.style.marginTop = '2.2em';
      pn.style.width = '100%';
      pn.style.maxWidth = '40em';
      var tag = el('div', 'mynote-tag', '✧ Your note');
      var ttl = el('div', 'mynote-title');
      var bod = el('div', 'mynote-body');
      var car = el('span', 'caret');
      add(pn.__body, tag, ttl, bod);
      bod.appendChild(car);
      add(scene, cap, pn);
      return { cap: cap, pn: pn, tag: tag, ttl: ttl, bod: bod, car: car };
    },
    draw: function (r, u) {
      drawCaption(r.cap, u, 0);
      rise(r.pn, beat(u, 0.7, 0.8), 30);
      rise(r.tag, beat(u, 1.1, 0.5), 10);

      var tp = p(u, 1.35, 0.85);
      r.ttl.textContent = D.MYNOTE.title.slice(0, Math.round(D.MYNOTE.title.length * tp));

      var bp = p(u, 2.15, 2.2);
      var txt = D.MYNOTE.body.slice(0, Math.round(D.MYNOTE.body.length * bp));
      r.bod.textContent = txt;
      r.bod.appendChild(r.car);
      /* Deterministic blink: derived from u, never from a wall clock. */
      var typing = u > 1.3 && u < 4.5;
  ...
  SCENES.push({
    ...
    draw: function (r, u) {
      ...
      /* The right answer resolves. */
      var pick = beat(u, 3.3, 0.45);
      var right = r.opts[D.MCQ.correct];
      right.classList.toggle('right', pick > 0.5);
      right.style.transform = 'translateY(0) scale(' + (1 + 0.02 * Math.sin(pick * Math.PI)) + ')';
      r.opts.forEach(function (o, i) {
        if (i !== D.MCQ.correct) o.style.opacity = 1 - 0.55 * pick;
      });
  ...
  SCENES.push({
    ...
    build: function (scene) {
      ...
      var rows = D.INVESTIGATION.rows.map(function (rw) {
        ...
        var td1 = el('td', rw[2] === 'high' ? 'high' : null, rw[1]);
